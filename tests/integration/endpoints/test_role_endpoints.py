...
from app.repositories import user_role_repo, permission_repo
...
class TestRoleEndpoints(BaseTestCase):
    ...
    def test_get_specific_role_endpoint(self):
        ...
        PermissionFactory.create(keyword="view_roles", role=role)
        UserRoleFactory.create(user_id=user_id, role=role)

        response = self.client().get(
            self.make_url("/roles/{}".format(role.id)), headers=self.headers()
        )
        response_json = self.decode_from_json_string(response.data.decode("utf-8"))
        payload = response_json["payload"]
