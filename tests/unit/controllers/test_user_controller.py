...
class TestUserController(BaseTestCase):
    ...
    def test_list_admin_users_ok_response(
        self, mock_get_user_by_email_or_id, mock_filter_by, mock_pagination_meta
    ):
        """
    ...
    @patch.object(UserController, "request_params")
    def test_create_user_method_handles_user_creation_with_duplicate_slack_id(
    ...
    def test_list_user_succeeds(self):
        with self.app.app_context():
            ...
            user = UserFactory(
                slack_id="-LXTuXlk2W4Gskt8KTte", user_type_id=user_role.id
            )
            ...
            self.assertEqual(
                response.get_json()["payload"]["user"]["slackId"], user.slack_id
            )
            self.assertEqual(
                response.get_json()["payload"]["user"]["firstName"], user.first_name
            )
            self.assertEqual(
    ...
    def test_list_user_when_user_found_succeeds(self):
        with self.app.app_context():
            ...
            response = user_controller.list_user(
                slack_id="-LXTuXlk2W4Gskt8KTtedhmdydsbnyw"
            )
