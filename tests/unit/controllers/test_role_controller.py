...
class TestRoleController(BaseTestCase):
    ...
    def test_update_role_when_role_doesnot_exist(
        ...
        with self.app.app_context():
            ...
            assert result.status_code == 400
            assert (
                result.get_json()["msg"] == "Invalid or incorrect " "role_id provided"
            )
    ...
    def test_delete_user_role_ok_response(
        self, mock_user_role_repo_get, mock_user_role_repo_update
    ):
        """Test delete_user_role when the role does not exist."""
        # Arrange
        with self.app.app_context():
    ...
    def test_get_single_permission_ok_response(self, mock_permission_repo_filter_by):
    ...
    def test_update_permission_when_permission_already_updated(
        ...
        with self.app.app_context():
            ...
            assert result.status_code == 400
            assert (
                result.get_json()["msg"] == "Permission with this name "
                "already exists"
            )
    ...
    def test_delete_role_permission_ok_response(
        self, mock_role_repo_update, mock_permission_repo_get
    ):
