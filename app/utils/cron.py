...
class MealSessionCron(object):
    ...
    def _complete_meal_session_construction_and_save(
        self, meal_sessions, date_time_to_use
    ):
        """
        ...
        :return:
        """
        meal_session_date = date(
            year=date_time_to_use.year,
    ...
    def job_to_schedule(self):
        ...
        with self.app.app_context():
            ...
            for location in locations:
                ...
                if any(
                    [
                        scheduler_datetime.year > location_datetime.year,
                        scheduler_datetime.month > location_datetime.month,
                        scheduler_datetime.day > location_datetime.day,
                    ]
                ):
                    self._complete_meal_session_construction_and_save(
                        meal_sessions, scheduler_datetime
                    )
                else:
                    self._complete_meal_session_construction_and_save(
                        meal_sessions, location_datetime
                    )

            print(
                "    *** Meal session Cron Completed last run at: ",
                str(scheduler_datetime),
            )
