...
class Security:
    EMAIL_REGEX = re.compile(
        r"^[\-a-zA-Z0-9_]+(\.[\-a-zA-Z0-9_]+)*@[\-a-z]+\.[\-a-zA-Z0-9_]+\Z",
        re.I | re.UNICODE,
    )

    URL_REGEX = re.compile(
        r"^(http(s)?:\/\/)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]"
        r"{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)$"
    )
                ...
                if invalid_query_keys:
                    return (
                        make_response(
                            jsonify(
                                {
                                    "msg": "Invalid keys {}. The supported keys are {}".format(
                                        invalid_query_keys, model_fields_camel
                                    )
                                }
                            )
                        ),
                    ...
                    return (
                        make_response(
                            jsonify(
                                {
                                    "msg": "Paramter value passed in does not match parameter-key type"
                                }
                            )
                        ),
                        400,
                    ...
                    if name.endswith("ted_at"):
                        try:
                            ...
                            return controller.handle_response(
                                f"Bad Request - '{name}' should be valid date. Format: YYYY-MM-DD",
                                status_code=400,
                            )
                        ...
                        jsonify(
                            {
                                ...
                                "values must be any of the following {}".format(
                                    value, key, enum_values
                                )
                            }
                        )
                    ),
            ...
            if not cls.EMAIL_REGEX.match(value):
                return (
                    make_response(
                        jsonify(
                            {
                                "msg": "Bad Request - '{}' is not a valid email address.".format(
                                    value
                                )
                            }
                        )
            ...
            if not re.match(cls.URL_REGEX, value):
                return (
                    make_response(
                        jsonify(
                            {
                                "msg": "Bad Request - '{}' is not a valid url.".format(
                                    value
                                )
                            }
                        )
