...
def add_activity(target, listener_type="insert"):
    ...
    target_as_dict["created_at"] = target_as_dict["created_at"].strftime(
        "%Y-%m-%dT%H:%M:%S.%f"
    )
    target_as_dict["updated_at"] = target_as_dict["updated_at"].strftime(
        "%Y-%m-%dT%H:%M:%S.%f"
    )
    ...
    listener_types = {"insert": "create", "update": "update", "delete": "delete"}

    action_type = listener_types.get(listener_type, "")

    channel = target.channel if hasattr(target, "channel") else None

    channel = "slack" if channel == "slack" else "web"

    if listener_type == "insert":
        action_detail["Current State"] = target_as_dict
        del action_detail["Previous State"]
        action_details = stringify_action_detail(
            user_id, model_name, action_detail, "created"
        )

    if listener_type == "update" and not target_as_dict.get("is_deleted"):
        action_detail["Current State"] = target_as_dict
        action_details = stringify_action_detail(
            user_id, model_name, action_detail, "updated"
        )

    if listener_type == "update" and target_as_dict.get("is_deleted"):
        action_detail["Soft deleted Entity"] = target_as_dict
        action_details = stringify_action_detail(
            user_id, model_name, action_detail, "soft deleted"
        )

    if listener_type == "delete":
        action_detail["Deleted Entity"] = target_as_dict
        del action_detail["Previous State"]
        action_detail["Current State"] = {}
        action_details = stringify_action_detail(
            user_id, model_name, action_detail, "hard deleted"
        )
