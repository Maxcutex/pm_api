...
class BaseRepo:
    ...
    def fetch_all_paginate(self):
        """Return all the data in the model with pagination"""
    ...
    def get_or_404(self, *args):
        ...
        raise BaseModelValidationError(
            msg="{} with id {} not found".format(self._model.__name__, args[0]),
            status_code=404,
        )
    ...
    def paginate(self, page=None, per_page=None, error_out=False):
        ...
        try:
            page = (
                BaseRepo._positive_int("page", page)
                if page
                else BaseRepo.DEFAULT_PAGINATION_PAGE_NUMBER
            )
            per_page = (
                BaseRepo._positive_int("per_page", per_page)
                if per_page
                else BaseRepo.DEFAULT_PAGINATION_PER_PAGE_NUMBER
            )
    ...
    def filter_by_desc(self, *args, **kwargs):
        ...
        return (
            self._model.query.filter_by(**kwargs)
            .order_by(desc(*args))
    ...
    def filter_by_query_params(self, query_params):
        ...
        return (
            self._model.query.filter(*filter_expression)
    ...
    def get_unpaginated_asc(self, *args, **kwargs):
        """Query and filter the data of the model in ascending order."""
        return self._model.query.filter_by(**kwargs).order_by(asc(*args)).all()
    ...
    @staticmethod
    def check_exists_else_where(
        model_class,
        first_attribute,
        first_compare_value,
        second_attribute,
        second_compare_value,
    ):
        """
