...
class {model_name}(BaseModel):
	...
	""".format(
        model_name=name_clean, table_name=inflect_engine.plural(name)
    )
...
def create_repo(name):
    name_clean = to_pascal_case(
        name
    )  #''.join(list(map(lambda x: x.capitalize(), name.split('_'))))
...
class {name_clean}Repo(BaseRepo):
	...
	def __init__(self):
		BaseRepo.__init__(self, {name_clean})""".format(
        name=name, name_clean=name_clean
    )
...
class {name_clean}Factory(factory.alchemy.SQLAlchemyModelFactory):
	...
	# cohort_position = fake_cohort_position
			...
			""".format(
        name_clean=name_clean, name=name
    )
...
if __name__ == "__main__":
    ...
    if command == "make:model" or command == "make:models":
        ...
        if "controller" in extras:
            ...
            print(
                colored(
                    "Controller: {}Controller Location: {}".format(c[0], c[1]), "green"
                )
            )
        ...
        if "repo" in extras:
            ...
            print(
                colored("Repository: {}Repo Location: {}".format(r[0], r[1]), "green")
            )
    ...
    if command == "make:controller" or command == "make:controllers":
        ...
        c = create_controller(name=name)
        print(
            colored("Controller: {}Controller Location: {}".format(c[0], c[1]), "green")
        )
